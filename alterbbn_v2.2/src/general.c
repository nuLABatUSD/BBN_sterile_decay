...
/* matrix inversion, input matrix of size n x n, output of the same size */
{
	...
	for (ie = 0; ie < 2*n; ie++) matrix[ie] = (double *) malloc(2*n * sizeof(double));

	for(ie=0;ie<n;ie++) for(je=0;je<n;je++) 
	{
		matrix[ie][je]=in[ie][je];
	}
	...
	double** product; /* product of size n * n */
	product = (double **) malloc(n * sizeof(double*));
	for (ie = 0; ie < n; ie++) product[ie] = (double *) malloc(n * sizeof(double));

	for(ie=0;ie<n;ie++) product[ie][ie]=0.;

	for(ie=0;ie<n;ie++) for(ke=0;ke<n;ke++) product[ie][ie]+=in[ie][ke]*out[ke][ie];

	for(ie=0;ie<n;ie++) if(fabs(product[ie][ie]-1.)<1.e-2) test*=1; else test=0; /* only check the diagonal */
	
	for (ie = 0; ie < 2*n; ie++) free(matrix[ie]);
	free(matrix);
	
	for(ie=0;ie<n;ie++) free(product[ie]);
	free(product);
...
int cholesky(int n, double** in, double** out)
{
    ...
    if(test==0)
    {
		...
		for (ie = 0; ie < n; ie++) free(reconstructed[ie]);
		free(reconstructed);
	}
